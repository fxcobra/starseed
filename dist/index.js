    ...
    try {
        ...
        if (buttons && buttons.length) {
            ...
            void sendVendorButtons(vendorId, to, text, list)
                ...
                .catch((e) => console.log(`vendor ${vendorId}: buttons error ${e?.message ?? String(e)}`));
            res.status(202).json({ ok: true, queued: true });
            return;
        }
        ...
        void sendVendorMessage(vendorId, to, text)
            ...
            .catch((e) => console.log(`vendor ${vendorId}: text error ${e?.message ?? String(e)}`));
        res.status(202).json({ ok: true, queued: true });
    }
    catch (e) {
...
app.post("/vendors/:vendorId/broadcast", async (req, res) => {
    ...
    const text = typeof req.body?.text === "string" ? req.body.text : "";
    const delaySecondsRaw = typeof req.body?.delay_seconds === "number"
...
app.post("/vendors/:vendorId/groups/lock-bulk", async (req, res) => {
    ...
    const delaySecondsRaw = typeof req.body?.delay_seconds === "number"
        ...
        : typeof req.body?.delaySeconds === "number"
            ...
            : 0;
    const delaySeconds = Number.isFinite(delaySecondsRaw) ? Math.max(0, Math.min(600, delaySecondsRaw)) : 0;
    const unlockAfterSecondsRaw = typeof req.body?.unlock_after_seconds === "number"
    ...
    const list = targets.filter((x) => typeof x === "string").map((x) => x.trim()).filter((x) => x !== "");
...
app.post("/vendors/:vendorId/drops", async (req, res) => {
    ...
    const delaySecondsRaw = typeof req.body?.delaySeconds === "number"
        ? req.body.delaySeconds
