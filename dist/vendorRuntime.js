                    ...
                    void (async () => {
                        ...
                        try {
                            if (runtime.baileysSend)
                                await runtime.baileysSend(sender, `Hi! To continue, please send: ${dmKeyword}`);
                        }
                        catch {
                            void 0;
                        }
                    ...
                    void (async () => {
                        ...
                        const openKeys = Array.isArray(cfg?.openrouter?.keys)
                            ? cfg.openrouter.keys
                                ...
                                .map((x) => x.trim())
                            : [];
                        const openModel = typeof cfg?.openrouter?.model === "string" ? String(cfg.openrouter.model).trim() : "";
                        ...
                        if (askDmOnly) {
                            try {
                                if (runtime.baileysSendGroupText)
                                    await runtime.baileysSendGroupText(groupId, `${mentionText}I can send options in DM. Please DM me with “${cleaned || "Menu"}” to continue.`, mentions);
                            }
                            catch {
                                void 0;
                            }
...
export async function sendVendorBroadcast(vendorId, recipients, text, delayMs) {
    const runtime = await startVendorRuntime(vendorId);
    if (!runtime.baileysSend)
        throw new Error("WhatsApp runtime not ready.");
