...
const paymentPollTimers = new Map();
...
export async function fetchVendorBotConfig(vendorId) {
    ...
    const brainStyle = brainStyleRaw === "straightforward" || brainStyleRaw === "interactive" || brainStyleRaw === "balanced" ? brainStyleRaw : "balanced";
    ...
    const delivery = json?.deliveryOptions ?? null;
    ...
    const methodsRaw = Array.isArray(delivery?.methods) ? delivery.methods : [];
    const deliveryMethods = methodsRaw
        ...
        return {
            ...
            areas: Array.isArray(o.areas) ? o.areas.filter((x) => typeof x === "string" && x.trim() !== "") : [],
    ...
    const cfg = {
        ...
        whatsappGroupSettings: {
            ...
            tagBack: typeof json?.whatsappGroupSettings?.tagBack === "boolean" ? Boolean(json.whatsappGroupSettings.tagBack) : true,
            searchMode: (() => {
                const sm = typeof json?.whatsappGroupSettings?.searchMode === "string" ? String(json.whatsappGroupSettings.searchMode).trim() : "dm_cta";
                if (sm === "in_group")
                    return "in_group";
                if (sm === "ask_dm")
                    return "ask_dm";
        ...
        whatsappHandoffSettings: {
            enabled: Boolean(json?.whatsappHandoffSettings?.enabled),
            pauseBot: Boolean(json?.whatsappHandoffSettings?.pauseBot),
            notifyPortal: typeof json?.whatsappHandoffSettings?.notifyPortal === "boolean" ? Boolean(json.whatsappHandoffSettings.notifyPortal) : true,
            notifyPeers: Array.isArray(json?.whatsappHandoffSettings?.notifyPeers)
                ? json.whatsappHandoffSettings.notifyPeers
                    .filter((x) => typeof x === "string" && String(x).trim())
                    .map((x) => x.trim())
                    .slice(0, 10)
                : [],
            stages: {
                view_product: Boolean(json?.whatsappHandoffSettings?.stages?.view_product),
                select_product: Boolean(json?.whatsappHandoffSettings?.stages?.select_product),
                select_variation: Boolean(json?.whatsappHandoffSettings?.stages?.select_variation),
                before_payment: Boolean(json?.whatsappHandoffSettings?.stages?.before_payment),
            },
        },
    };
...
async function fetchCustomerProfile(vendorId, peer) {
    const base = apiBase();
    const secret = (process.env.BOT_SERVER_SECRET ?? "").trim();
    ...
    return {
        ...
        delivery_method_name: typeof cust.delivery_method_name === "string" && cust.delivery_method_name.trim() ? cust.delivery_method_name.trim() : null,
    };
}
async function upsertCustomerProfile(vendorId, payload) {
    const base = apiBase();
    const secret = (process.env.BOT_SERVER_SECRET ?? "").trim();
...
async function createHandoff(vendorId, payload) {
    const base = apiBase();
    const secret = (process.env.BOT_SERVER_SECRET ?? "").trim();
...
function clearCheckout(session) {
    session.cart = null;
...
async function verifyMarketplacePayment(reference, email) {
    const url = `${apiBase()}/marketplace/orders/by-reference/${encodeURIComponent(reference)}/verify-payment`;
    return await postJson(url, { email }, {}, 20_000).catch(() => ({ ok: false, status: 0, json: null }));
}
async function fetchStoreSlug(vendorId) {
    ...
    const { ok, json } = await fetchJson(url, 4_000).catch(() => ({ ok: false, status: 0, json: null }));
    if (!ok || !json || typeof json !== "object")
        return null;
...
export async function fetchProductsForVendor(vendorId) {
    if (productsSource === "supabase" && supabase) {
        ...
        let q = supabase.from("products_with_variations").select("*").order("created_at", { ascending: false });
        ...
        for (const row of rows) {
            ...
            const imgsRaw = Array.isArray(row?.images) ? row.images : [];
            const imgs = imgsRaw
                .filter((x) => typeof x === "string" && x.trim() !== "")
                .map((x) => absolutizeMediaUrl(x));
            const vidsRaw = Array.isArray(row?.videos) ? row.videos : [];
            const vids = vidsRaw
                .filter((x) => typeof x === "string" && x.trim() !== "")
                .map((x) => absolutizeMediaUrl(x));
            ...
            const varsRaw = Array.isArray(row?.variations) ? row.variations : Array.isArray(row?.product_variations) ? row.product_variations : [];
            const variations = [];
            for (const v of varsRaw) {
                const vid = typeof v?.id === "number" ? v.id : Number(v?.id);
                const vname = typeof v?.name === "string" ? String(v.name).trim() : "";
                const vPriceCents = typeof v?.price_cents === "number" ? v.price_cents : Number(v?.price_cents);
                const vPriceRaw = typeof v?.price === "number" ? v.price : Number(v?.price);
                ...
                const vImgsRaw = Array.isArray(v?.images) ? v.images : [];
        ...
        try {
            ...
            if (store && store.slug) {
                ...
                const { ok, json } = await fetchJson(url, 6_000).catch(() => ({ ok: false, status: 0, json: null }));
                const apiProducts = ok && json && typeof json === "object" ? json?.products : null;
    ...
    for (const p of products) {
        ...
        const imgs = imgsRaw
            .filter((x) => typeof x === "string" && x.trim() !== "")
            .map((x) => absolutizeMediaUrl(x));
        const vidsRaw = Array.isArray(p?.videos) ? p.videos : [];
        const vids = vidsRaw
            .filter((x) => typeof x === "string" && x.trim() !== "")
            .map((x) => absolutizeMediaUrl(x));
        const image0 = typeof imgs?.[0] === "string" ? String(imgs[0]).trim() : "";
        ...
        const varsRaw = Array.isArray(p?.variations) ? p.variations : [];
        const variations = [];
        for (const v of varsRaw) {
            const vid = typeof v?.id === "number" ? v.id : Number(v?.id);
            const vname = typeof v?.name === "string" ? String(v.name).trim() : "";
...
function extractOpenRouterContent(json) {
    ...
    if (typeof content === "string" && content.trim())
        return { content: content.trim(), tool_calls, debug: null };
    if (Array.isArray(content)) {
        ...
        if (joined)
            return { content: joined, tool_calls, debug: null };
    }
    if (tool_calls)
        return { content: "", tool_calls, debug: null };
    ...
    return { content: null, debug: hint };
}
function normalizeOpenRouterMessages(messages, cacheControl, isAnthropic) {
    const list = Array.isArray(messages) ? messages : [];
    if (!cacheControl || !isAnthropic)
        return list;
    return list.map((m) => {
        ...
        if (typeof c !== "string")
            return m;
        if (role === "system")
            return { ...m, content: [{ type: "text", text: c, cache_control: cacheControl }] };
        if (role === "user" || role === "assistant")
            return { ...m, content: [{ type: "text", text: c }] };
        return m;
    });
}
function openRouterTimeoutMsForModel(model) {
...
async function openRouterChatOneAdvanced(key, model, payload) {
    const k = (key ?? "").trim();
    if (!k)
        return { ok: false, status: 0, message: "OpenRouter API key is missing." };
    ...
    const cacheControl = cacheEnabled ? { type: "ephemeral", ttl: "1h" } : null;
    ...
    const state = openRouterChatOneAdvanced._circuit;
    const now = Date.now();
    ...
    openRouterChatOneAdvanced._circuit = circuit;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            ...
            if (resp.status < 200 || resp.status >= 300) {
                const msgFromJson = typeof json?.error?.message === "string"
...
function shouldCircuitOpen(state, now) {
...
async function openRouterChatOne(key, model, systemPrompt, userText) {
...
function looksLikeKeyFailure(r) {
    if (r.ok)
        return false;
...
async function openRouterChatWithKeys(keys, model, systemPrompt, userText) {
    const list = (Array.isArray(keys) ? keys : []).map((k) => (k ?? "").trim()).filter(Boolean);
    if (!list.length)
        return { ok: false, status: 0, message: "No OpenRouter API keys configured." };
    let last = null;
    for (let i = 0; i < list.length; i++) {
        const r = await openRouterChatOne(list[i], model, systemPrompt, userText);
        if (r.ok)
            return r;
        last = r;
        if (!looksLikeKeyFailure(r))
            return r;
    }
...
function mostCommonCurrency(products) {
...
function parseConfirmDecision(text) {
    ...
    if (!t)
        return null;
    if (t === "1" || t === "yes" || t === "y" || t === "ok" || t === "okay" || t === "sure" || t === "go ahead")
        return "yes";
    if (t.includes("yes") || t.includes("show") || t.includes("send") || t.includes("that is it") || t.includes("thats it"))
        return "yes";
    if (t === "2" || t === "no" || t === "n")
        return "no";
    if (t.includes("no") || t.includes("change") || t.includes("edit") || t.includes("not that") || t.includes("wrong"))
        return "no";
    return null;
}
function looksLikeCheckPermissionPrompt(text) {
    const t = normText(text);
    if (!t)
        return false;
    if (!t.includes("check my system"))
        return false;
    if (!t.includes("would you like"))
        return false;
    if (t.includes("pull up") || t.includes("show") || t.includes("options"))
        return true;
    return false;
}
function simpleLocalSearch(products, query) {
    ...
    for (const p of Array.isArray(products) ? products : []) {
        ...
        for (const tok of tokens) {
            if (hay.includes(tok))
                score += 1;
            if (normText(p.name ?? "").includes(tok))
                score += 2;
            if (normText(p.category ?? "").includes(tok))
                score += 1;
        }
        if (score > 0)
            scored.push({ p, score });
...
function parsePriceConstraint(text) {
    ...
    const between = raw.match(/between\s+(?:[a-z]{2,4}|[€£$₦₵])?\s*(\d[\d,]{1,7}(?:\.\d{1,2})?k?)\s*(?:and|to|-|–)\s*(?:[a-z]{2,4}|[€£$₦₵])?\s*(\d[\d,]{1,7}(?:\.\d{1,2})?k?)/);
    if (between) {
    ...
    const dash = raw.match(/(?:^|\s)(?:[a-z]{2,4}|[€£$₦₵])?\s*(\d[\d,]{1,7}(?:\.\d{1,2})?k?)\s*(?:-|–)\s*(?:[a-z]{2,4}|[€£$₦₵])?\s*(\d[\d,]{1,7}(?:\.\d{1,2})?k?)(?:\s|$)/);
    if (dash) {
...
export function createEllbot(deps) {
    const userSessions = {};
    const openRouterErrorThrottle = new Map();
    let openRouterCursor = 0;
    const vendorProductsCache = new Map();
    const handoffControls = new Map();
    ...
    function rememberHandoffToken(token, peer) {
        if (!token || !peer)
            return;
        const now = nowMs();
        ...
        for (const [k, v] of handoffControls.entries()) {
            if (!v || typeof v.at !== "number" || now - v.at > maxAgeMs)
                handoffControls.delete(k);
        }
    }
    async function sendHandoffNotifyCard(to, body, dmUrl, token) {
        ...
        await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Handoff relay timed out.");
    }
    function isExcludedPeer(to, excluded) {
    ...
    async function loadSessionsFromDisk() {
        try {
            ...
            const sessions = (json && typeof json === "object" ? json.sessions : null);
            ...
            for (const [jid, v] of entries) {
                if (!jid || typeof v !== "object" || !v)
                    continue;
                const lastAt = typeof v.lastAt === "number" ? v.lastAt : Number(v.lastAt);
                if (Number.isFinite(lastAt) && lastAt > 0 && now - lastAt > maxAgeMs)
                    continue;
                const stateRaw = typeof v.state === "string" ? v.state : "chat";
                const state = stateRaw === "awaiting_qty" ||
                ...
                userSessions[jid] = {
                    ...
                    paymentLink: v.paymentLink && typeof v.paymentLink === "object"
                        ? {
                            url: typeof v.paymentLink.url === "string" ? String(v.paymentLink.url) : "",
                            ref: typeof v.paymentLink.ref === "string" ? String(v.paymentLink.ref) : "",
                            at: typeof v.paymentLink.at === "number" ? v.paymentLink.at : Number(v.paymentLink.at),
                        }
                        : undefined,
    ...
    async function ensureSessionsLoaded() {
        ...
        if (sessionsLoadPromise)
            return await sessionsLoadPromise;
    ...
    async function getOpenRouterConfig() {
        ...
        const modelFromVendor = typeof cfg?.openrouter?.model === "string" ? String(cfg.openrouter.model).trim() : "";
    ...
    async function openRouterChatVendor(arg1, arg2) {
        ...
        const payload = typeof arg1 === "string"
            ...
            : arg1;
        if (openRouterCursor >= keys.length)
            openRouterCursor = 0;
        ...
        for (let i = 0; i < ordered.length; i++) {
            ...
            const r = await openRouterChatOneAdvanced(ordered[i], model, payload);
            if (!r.ok && modelFromVendor && model === modelFromVendor && fallbackModel && fallbackModel !== model && looksLikeModelNotFound(r)) {
                const retry = await openRouterChatOneAdvanced(ordered[i], fallbackModel, payload);
                if (retry.ok) {
                    openRouterCursor = idx;
                    return retry;
                }
                last = retry;
                if (!looksLikeKeyFailure(retry))
                    return retry;
    ...
    function pushHistory(session, dir, text) {
    ...
    function formatHistoryForAI(session, turns) {
    ...
    async function withLocalTimeout(p, ms, message) {
        return await deps.withTimeout(p, ms, message);
    }
    async function sendText(to, text) {
        await deps.withTimeout(deps.sock.sendMessage(to, { text }), 25_000, "Send timed out.");
    }
    async function sendWithMenu(to, session, text, footerText) {
        const choices = session.cart
            ? [
    ...
    async function sendMenuAndRemember(to, session, text, footerText) {
    ...
    async function sendAndRemember(to, session, text) {
        await sendWithCancel(to, session, text);
        pushHistory(session, "out", text);
    }
    async function maybeTriggerHandoff(to, session, vendorCfg, storeName, stage, context) {
        const hs = vendorCfg?.whatsappHandoffSettings;
        if (!hs || !hs.enabled)
            return false;
        const enabled = stage === "view_product"
        ...
        scheduleSave();
        await sendChoiceCard(to, "I’ve handed this chat over to an assistant.\n\nI won’t reply until you tap *Continue with Bot* or type *Menu*.", [
        ...
        void (async () => {
            ...
            if (customerName)
                msgParts.push(`Name: ${customerName}`);
            const title = typeof context?.title === "string" ? String(context.title).trim() : "";
            if (title)
                msgParts.push(`Item: ${title.slice(0, 80)}`);
    ...
    async function smartSearch(query, vendorId, allProducts, opts) {
        void vendorId;
        ...
        if (brand)
            filtered = filtered.filter((p) => (p.brand ?? "").trim().toLowerCase() === brand);
        ...
        if (excludeCategory)
            filtered = filtered.filter((p) => (p.category ?? "").toLowerCase() !== excludeCategory);
        const min = typeof opts?.min === "number" && Number.isFinite(opts.min) ? opts.min : null;
        if (min !== null)
            filtered = filtered.filter((p) => typeof p.price === "number" && Number.isFinite(p.price) && p.price >= min);
        const max = typeof opts?.max === "number" && Number.isFinite(opts.max) ? opts.max : null;
        if (max !== null)
            filtered = filtered.filter((p) => typeof p.price === "number" && Number.isFinite(p.price) && p.price <= max);
        if (!q)
        ...
        for (const p of filtered) {
            ...
            for (const tok of tokens) {
                ...
                if (exactRe.test(catHay))
                    score += 20;
                if (exactRe.test(brandHay))
                    score += 100;
    ...
    function agentMessagesFromSession(session, memoryTurns) {
    ...
    function looksLikeToolsUnsupported(r) {
        if (r.ok)
            return false;
        if (r.status !== 400)
            return false;
    ...
    function looksLikeModelNotFound(r) {
        if (r.ok)
            return false;
    ...
    function looksLikeShoppingText(text) {
        const t = (text ?? "").toLowerCase();
        if (!t.trim())
            return false;
        if (t.includes("menu") || t.includes("catalog"))
            return true;
        if (t.includes("show me") || t.includes("what do you have") || t.includes("available"))
            return true;
        if (/\b(buy|order|price|cost|budget|under|below|above|between)\b/.test(t))
            return true;
        if (/\b(i need|need a|need an|i want|want a|want an|looking for|do you have)\b/.test(t))
            return true;
        return false;
    }
    ...
    async function aiPlanCatalogSearchNoTools(args) {
        ...
        if (!parsed || typeof parsed !== "object")
            return null;
        const query = typeof parsed.query === "string" ? String(parsed.query).trim() : "";
        const negative_keywords = Array.isArray(parsed.negative_keywords)
            ? parsed.negative_keywords
                ...
                .slice(0, 20)
            : [];
        const brand = typeof parsed.brand === "string" ? String(parsed.brand).trim() : "";
        ...
        const intro_message = typeof parsed.intro_message === "string" ? String(parsed.intro_message).trim() : "";
        if (!query && !brand)
            return null;
    ...
    async function runAgenticTurn(args) {
        const cfg = await getOpenRouterConfig();
        const aiTimeoutMs = openRouterTimeoutMsForModel(cfg.model) + 5_000;
        const styleGuide = args.assistantStyle === "interactive"
        ...
        const tools = [
            {
                ...
                function: {
                    name: "search_catalog",
                    description: "Search the database and display interactive product cards. Call this ONLY after the user explicitly agrees to let you check the system/show products.",
                    parameters: {
                        type: "object",
                        properties: {
                            ...
                            negative_keywords: {
                                type: "array",
                                items: { type: "string" },
                                description: "CRITICAL: Dynamically list words to exclude accessories or irrelevant items based on the context. (e.g., if they want a phone, exclude 'case', 'charger', 'light'. If they want a dress, exclude 'belt', 'bag').",
                            },
                            min_price: { type: "number" },
                            max_price: { type: "number" },
                            intro_message: {
                                type: "string",
                                description: "Write a short, engaging header to introduce the products based on the conversation (e.g., 'I checked the system, here are the options we have!').",
                            },
                        },
        ...
        let messages = [{ role: "system", content: systemInstruction }, ...agentMessagesFromSession(args.session, args.memoryTurns)];
        const maxRounds = 1;
        for (let round = 0; round < maxRounds; round++) {
            const resp = await withLocalTimeout(openRouterChatVendor({ messages, tools, temperature: 0.3, max_tokens: 450 }), aiTimeoutMs, "AI timed out.");
            if (!resp.ok) {
            ...
            const min_price = typeof parsed.min_price === "number" && Number.isFinite(parsed.min_price) ? parsed.min_price : null;
            const max_price = typeof parsed.max_price === "number" && Number.isFinite(parsed.max_price) ? parsed.max_price : null;
            const intro = typeof parsed.intro_message === "string" ? String(parsed.intro_message).trim() : "";
    ...
    function productUnitPrice(product, variation) {
        if (variation && typeof variation.price === "number" && Number.isFinite(variation.price))
            return variation.price;
        return Number.isFinite(product.price) ? product.price : 0;
    }
    function cartTitle(product, variation) {
        return variation ? `${product.name} — ${variation.name}` : product.name;
    }
    function buildCartItem(product, variation, qty) {
    ...
    async function ensureValidQtyOrPrompt(to, session) {
        if (!session.cart)
            return true;
    ...
    async function sendPaymentOptions(to, cart, delivery) {
        ...
        for (let i = 0; i < allButtons.length; i += 3) {
            ...
            await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Payment options relay timed out.");
            if (i + 3 < allButtons.length)
    ...
    function looksLikeStoreName(name, storeName) {
        ...
        if (n === sn)
            return true;
        if (sn.length >= 4 && (n.includes(sn) || sn.includes(n)))
            return true;
        return false;
    }
    function resolveCustomerName(session, storeName) {
        const saved = (session.customerName ?? "").trim();
        if (saved && !looksLikeStoreName(saved, storeName))
            return saved.slice(0, 200);
        const wa = (session.customerPushName ?? "").trim();
        if (wa && !looksLikeStoreName(wa, storeName))
            return wa.slice(0, 200);
        return "";
    }
    async function sendCheckoutReview(to, session, storeName) {
        ...
        if (!isPickup)
            choices.push({ label: "Change Address", id: "CHANGE_ADDRESS" });
    ...
    async function promptAddress(to, session, storeName) {
        const savedAddr = (session.deliveryAddress ?? "").trim();
        if (savedAddr) {
            await sendChoiceCard(to, `Saved delivery address:\n${savedAddr.slice(0, 220)}\n\nUse this address?`, [
    ...
    async function promptName(to, session, storeName) {
        ...
        if (savedName) {
            await sendChoiceCard(to, `Saved name:\n${savedName}\n\nUse this name for your order?`, [
        ...
        if (waName) {
            await sendChoiceCard(to, `WhatsApp name:\n${waName}\n\nUse this name for your order?`, [
    ...
    async function initiateMarketplaceCheckout(to, session, storeName) {
        ...
        const body = {
            ...
            shipping: {
                ...
                fee_cents: typeof session.deliveryFeeCents === "number" && Number.isFinite(session.deliveryFeeCents) ? Math.max(0, Math.round(session.deliveryFeeCents)) : 0,
        ...
        if (!ok) {
            const msg = typeof json?.message === "string" ? json.message : "Checkout failed.";
            throw new Error(msg);
        }
        const authorizationUrl = json?.authorization_url ?? json?.data?.authorization_url ?? null;
        if (typeof authorizationUrl !== "string" || !authorizationUrl.trim()) {
            const msg = typeof json?.message === "string" ? json.message : "Payment link unavailable.";
            throw new Error(msg);
        }
        const reference = typeof json?.reference === "string" ? String(json.reference).trim() : "";
        if (reference)
            session.lastReference = reference;
    ...
    async function prepareImageMedia(url, label, timeoutMessage) {
        const primary = absolutizeMediaUrl(url) || placeholderUrl(label);
        try {
            return await deps.withTimeout(Promise.resolve(deps.prepareWAMessageMedia({ image: { url: primary } }, { upload: deps.sock.waUploadToServer })), 25_000, timeoutMessage);
        }
        catch (e) {
            const msg = e instanceof Error ? e.message : "Media prepare failed.";
            if (!/\b404\b/.test(msg) && !/status code 404/i.test(msg))
                deps.onError(msg);
            const fallback = placeholderUrl(label);
            return await deps.withTimeout(Promise.resolve(deps.prepareWAMessageMedia({ image: { url: fallback } }, { upload: deps.sock.waUploadToServer })), 25_000, timeoutMessage);
        }
    }
    async function sendProductCards(to, products, slideCount) {
        const limit = Number.isFinite(slideCount) ? Math.max(1, Math.min(10, Math.floor(slideCount))) : 10;
        const byId = new Map();
        for (const p of Array.isArray(products) ? products : []) {
            if (p && typeof p.id === "number" && !byId.has(p.id))
                byId.set(p.id, p);
        }
        ...
        for (const p of list) {
            ...
            await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Product relay timed out.");
            await new Promise((r) => setTimeout(r, 600));
        }
    }
    function setLastCatalog(session, ids, per, title) {
    ...
    function getCatalogPageCount(c) {
        const total = Array.isArray(c.ids) ? c.ids.length : 0;
        const per = Number.isFinite(c.per) ? Math.max(1, Math.floor(c.per)) : 5;
        return Math.max(1, Math.ceil(total / per));
    }
    async function sendCatalogNav(to, page, pages) {
        const p = Number.isFinite(page) ? Math.max(0, Math.floor(page)) : 0;
        const total = Number.isFinite(pages) ? Math.max(1, Math.floor(pages)) : 1;
        const buttons = [];
        if (p > 0)
            buttons.push({ name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "Prev", id: "CAT_PREV" }) });
        if (p + 1 < total)
            buttons.push({ name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "Next", id: "CAT_NEXT" }) });
        ...
        await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Navigation relay timed out.");
    }
    async function sendChoiceCard(to, body, choices, footerText, media) {
    ...
    function startMomoAutoVerify(to, session, storeName) {
        const ref = (session.lastReference ?? "").trim();
        const email = (session.email ?? "").trim();
        if (!ref || !email)
            return;
        const existing = paymentPollTimers.get(to);
        if (existing) {
            try {
        ...
        if (!session.paymentPoll || session.paymentPoll.ref !== ref) {
            session.paymentPoll = { ref, startedAt: Date.now(), tries: 0 };
        }
        scheduleSave();
        const attempt = async () => {
            const poll = session.paymentPoll;
            if (!poll || poll.ref !== ref)
                return;
            if (session.state !== "awaiting_momo_confirmation" && session.state !== "awaiting_payment_confirmation")
                return;
            if (Date.now() - poll.startedAt > 120_000 || poll.tries >= 18) {
                session.paymentPoll = undefined;
                scheduleSave();
                await sendChoiceCard(to, `⏳ Still waiting for payment confirmation.\n\nIf the prompt doesn’t arrive, use *Payment Link* instead.`, [
            ...
            poll.tries += 1;
            scheduleSave();
            const { ok, json } = await verifyMarketplacePayment(ref, email);
            if (ok) {
                const status = typeof json?.order?.status === "string" ? String(json.order.status) : "processing";
                session.state = "chat";
            ...
            const t = setTimeout(() => void attempt(), 6500);
            paymentPollTimers.set(to, t);
        };
        const t = setTimeout(() => void attempt(), 3500);
        paymentPollTimers.set(to, t);
    }
    ...
    async function sendCatalogPage(to, session, products, page) {
    ...
    async function sendActionCarousel(to, title, cardsIn, slideCount) {
        const limit = Number.isFinite(slideCount) ? Math.max(1, Math.min(10, Math.floor(slideCount))) : 10;
        const list = cardsIn
            .slice(0, limit)
            .map((c) => ({
        ...
        for (let i = 0; i < allButtons.length; i += 3) {
            ...
            await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Action menu relay timed out.");
            if (i + 3 < allButtons.length)
                await new Promise((r) => setTimeout(r, 600));
        }
    }
    async function sendAiUnavailableMenu(to, session, storeName, shopPhone, slideCount) {
    ...
    async function sendDeliveryPicker(to, session, storeName, delivery, slideCount) {
        ...
        for (let i = 0; i < allButtons.length; i += 3) {
            ...
            await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Delivery picker relay timed out.");
            if (i + 3 < allButtons.length)
                await new Promise((r) => setTimeout(r, 600));
        }
    }
    function getVendorCategories(products) {
        const counts = new Map();
        for (const p of Array.isArray(products) ? products : []) {
    ...
    function getVendorBrands(products, category) {
        const counts = new Map();
        const catKey = (category ?? "").trim().toLowerCase();
        for (const p of Array.isArray(products) ? products : []) {
            const c = (typeof p?.category === "string" ? p.category.trim() : "").trim();
            if (catKey && c.trim().toLowerCase() !== catKey)
                continue;
    ...
    async function sendButtonsOnlyChunks(to, firstText, nextText, options, footerText) {
        const list = Array.isArray(options) ? options.filter((o) => o && o.id && o.label) : [];
        if (!list.length)
            return;
        const chunkSize = 3;
        for (let i = 0; i < list.length; i += chunkSize) {
            ...
            await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Buttons relay timed out.");
            if (i + chunkSize < list.length)
                await new Promise((r) => setTimeout(r, 600));
        }
    }
    async function sendMenuCategories(to, session, products, storeName) {
    ...
    async function sendMenuCategoryActions(to, session, products, storeName, category) {
        ...
        scheduleSave();
        const choices = [{ label: "Products", id: "MENU_CAT_PRODUCTS" }];
        if (brands.length)
            choices.push({ label: "Brands", id: "MENU_CAT_BRANDS" });
    ...
    async function sendMenuBrands(to, session, storeName) {
    ...
    async function showMenuProducts(to, session, products, storeName, cfg) {
    ...
    async function sendAiFallbackCategories(to, session, products, catalogCount, slideCount) {
        session.aiFallbackAt = nowMs();
        scheduleSave();
        if (!products.length) {
        ...
        for (let i = 0; i < pages; i++) {
            ...
            if (!chunk.length)
                break;
            await sendActionCarousel(to, `Categories ${i + 1}/${pages}`, chunk.map((c) => ({
    ...
    async function sendVariationPicker(to, product, page) {
        void page;
        const vs = Array.isArray(product.variations) ? product.variations : [];
        ...
        const safeDesc = (product.description ?? "").trim().slice(0, 700);
        const text = (`*${(product.name ?? "").trim().slice(0, 60)}*\n` +
            `💰 ${productPriceLabel(product)}\n\n` +
            `${safeDesc}\n\n` +
            `*Please tap your choice below:*`).slice(0, 1024);
        const media = await prepareImageMedia(product.image_url, product.name, "Product media prepare timed out.");
        const allButtons = [
            ...list.map((v) => ({
                name: "quick_reply",
                buttonParamsJson: JSON.stringify({
                    display_text: String(v.name ?? "").trim().slice(0, 20) || "Select",
                    id: `SELECT_VARIATION ${product.id} ${v.id}`,
                }),
            })),
        ...
        for (let i = 0; i < allButtons.length; i += 3) {
            ...
            await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Variation picker relay timed out.");
            if (i + 3 < allButtons.length)
                await new Promise((r) => setTimeout(r, 600));
        }
    }
    async function sendVariationDetails(to, product, variation) {
        ...
        await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Variation details relay timed out.");
    }
    async function sendProductDetails(to, product) {
        ...
        const remainingVideos = (Array.isArray(product.videos) ? product.videos : [])
            ...
            .filter((x) => x && x !== used);
        const queue = [];
        for (const u of remainingImages)
            queue.push({ kind: "image", url: u });
        for (const u of remainingVideos)
            queue.push({ kind: "video", url: u });
        const uniq = new Set();
        const capped = queue
            .filter((m) => (uniq.has(`${m.kind}:${m.url}`) ? false : (uniq.add(`${m.kind}:${m.url}`), true)))
            .slice(0, 4);
        if (capped.length > 1) {
            const album = capped.map((m) => (m.kind === "video" ? { video: { url: m.url } } : { image: { url: m.url } }));
            try {
                await deps.withTimeout(deps.sock.sendMessage(to, { album }, { delayMs: 400 }), 60_000, "Album send timed out.");
            }
            catch (e) {
                ...
                for (const m of capped) {
                    try {
                        if (m.kind === "video") {
                            await deps.withTimeout(deps.sock.sendMessage(to, { video: { url: m.url } }), 30_000, "Video send timed out.");
                        }
                        else {
                            await deps.withTimeout(deps.sock.sendMessage(to, { image: { url: m.url } }), 30_000, "Image send timed out.");
                        }
                        await new Promise((r) => setTimeout(r, 600));
                    }
                    catch (e2) {
        ...
        await deps.withTimeout(deps.sock.relayMessage(to, msg.message, { messageId: msg.key?.id }), 25_000, "Details relay timed out.");
    }
    async function sendStatusForwardProduct(to, product) {
        ...
        await deps.withTimeout(deps.sock.sendMessage(to, { image: { url }, caption }), 30_000, "Status forward send timed out.");
    }
    ...
    function detectSortIntent(text, cfg) {
        const t = (text ?? "").toLowerCase();
        if (cfg.allowRandom && (t.includes("random") || t.includes("surprise")))
            return "random";
        if (cfg.allowPriceSort &&
        ...
        if (cfg.allowPriceSort &&
            ...
            return "price_desc";
        if (cfg.allowLatest && (t.includes("latest") || t.includes("newest") || t.includes("recent")))
            return "latest";
        return null;
    }
    function detectCategoryIntent(text, products) {
        const t = (text ?? "").toLowerCase();
        if (!t)
            return null;
        ...
        for (const c of cats) {
            ...
            variants.add(lc);
            if (lc.endsWith("s") && lc.length > 3)
                variants.add(lc.slice(0, -1));
            if (!lc.endsWith("s"))
                variants.add(`${lc}s`);
    ...
    function applySort(products, mode) {
    ...
    async function sendMenuCatalog(to, session, products, cfg, title) {
        ...
        setLastCatalog(session, picked.map((p) => p.id), per, typeof title === "string" && title.trim() ? title.trim() : "Catalog");
        await sendCatalogPage(to, session, products, 0);
    }
    async function handleDm(to, rawText, pushName) {
        ...
        if (pn)
            session.customerPushName = pn.slice(0, 200);
        ...
        if (upperTextRaw.startsWith("HANDOFF_BOT_ON:") || upperTextRaw.startsWith("HANDOFF_BOT_OFF:") || upperTextRaw.startsWith("HANDOFF_VIEW_CHAT:")) {
            ...
            if (!userSessions[customerPeer])
                userSessions[customerPeer] = { state: "chat", cart: null, greetedAt: 0, lastNudgeAt: 0, lastNudgeSig: "" };
        ...
        const menuAgeOk = typeof session.menu?.at === "number" && Number.isFinite(session.menu.at) && nowMs() - session.menu.at < 30 * 60_000;
        if (menuAgeOk) {
        ...
        const slideCount = typeof catalogCfg?.itemsPerPage === "number"
            ? catalogCfg.itemsPerPage
            : typeof catalogCfg?.carouselSlideCount === "number"
                ? catalogCfg.carouselSlideCount
                : 10;
        ...
        const assistantStyle = brain?.assistantStyle === "straightforward" || brain?.assistantStyle === "interactive" || brain?.assistantStyle === "balanced"
        ...
        if (upperText === "CONFIRM_PAID" || upperText.startsWith("PAID")) {
            ...
            try {
                ...
                if (!ok) {
                    const msg = typeof json?.message === "string" ? json.message : "Payment verification failed.";
                    await sendWithCancel(to, session, `❌ ${msg}\n\nIf you’ve already paid, try again in a minute.`);
                    return;
                }
                const status = typeof json?.order?.status === "string" ? String(json.order.status) : "updated";
                session.state = "chat";
                clearCheckout(session);
                scheduleSave();
                await sendWithMenu(to, session, `✅ Payment confirmed.\nStatus: ${status}\nRef: ${ref}\n\nTap Menu to continue.`, storeName);
        ...
        if (session.state === "chat") {
            if (isGreeting(text)) {
                ...
                if (!last || nowMs() - last > 2 * 60 * 60_000) {
                    ...
                    const msg = typeof brain?.greeting === "string" && brain.greeting.trim()
        ...
        if (session.state === "awaiting_delivery_method") {
            ...
            if (!(await ensureValidQtyOrPrompt(to, session)))
                return;
        ...
        if (upperText === "PAY_DIRECT") {
            ...
            session.state = "awaiting_momo_provider";
            scheduleSave();
            await sendChoiceCard(to, "Select your Mobile Money network:", [
                { label: "MTN MoMo", id: "MOMO_PROVIDER_MTN" },
                { label: "Telecel Cash", id: "MOMO_PROVIDER_VOD" },
                { label: "AirtelTigo Money", id: "MOMO_PROVIDER_ATL" },
            ], storeName);
            return;
        }
        ...
        if (session.state === "awaiting_momo_confirmation") {
        ...
        if (session.state === "awaiting_momo_number") {
            ...
            try {
                ...
                if (!email)
                    throw new Error("Missing email.");
                const momo = normalizeGhanaPhone((session.momoNumber ?? "").trim());
                const body = {
                    store_slug: store.slug,
                    name: name || undefined,
                    email,
                    momo,
                    provider: (session.momoProvider ?? "mtn").trim() || "mtn",
                    shipping: {
                        method: typeof session.deliveryMethodId === "string" && session.deliveryMethodId.trim() ? session.deliveryMethodId.trim() : undefined,
                        fee_cents: typeof session.deliveryFeeCents === "number" && Number.isFinite(session.deliveryFeeCents) ? Math.max(0, Math.round(session.deliveryFeeCents)) : 0,
                        address: typeof session.deliveryAddress === "string" && session.deliveryAddress.trim() ? session.deliveryAddress.trim() : undefined,
                ...
                const { ok, json } = await postJson(url, body, {}, 25_000);
                if (!ok) {
                    const msg = typeof json?.message === "string" ? json.message : "MoMo charge failed.";
                    throw new Error(msg);
                }
                const reference = typeof json?.reference === "string" ? String(json.reference).trim() : "";
                if (reference)
                    session.lastReference = reference;
                const display = typeof json?.display_text === "string" ? String(json.display_text).trim() : "";
                const chargeStatus = typeof json?.charge_status === "string" ? String(json.charge_status).trim().toLowerCase() : "";
                ...
                if (chargeStatus === "send_pin") {
                    session.state = "awaiting_payment_confirmation";
                    scheduleSave();
                    await sendChoiceCard(to, `This payment needs an extra step we can’t complete here.\n\nPlease use *Payment Link* instead.\n\nRef: ${reference || "N/A"}`, [
                        { label: "Payment Link", id: "PAY_LINK" },
                        { label: "Cancel Order", id: "CANCEL_CHECKOUT" },
                        { label: "Menu", id: "SHOW_CATALOG" },
                    ], storeName);
                ...
                startMomoAutoVerify(to, session, storeName);
                await sendChoiceCard(to, `✅ A payment prompt has been sent to your phone.\nRef: ${reference || "N/A"}${display ? `\n\n${display}` : ""}\n\nApprove it on your phone, then confirm here.`, [
                    { label: "I've Paid", id: "CONFIRM_PAID" },
                    { label: "Payment Link", id: "PAY_LINK" },
                    { label: "Cancel Order", id: "CANCEL_CHECKOUT" },
                    { label: "Menu", id: "SHOW_CATALOG" },
        ...
        if (session.state === "awaiting_momo_otp") {
            ...
            if (!ref || !email) {
                session.state = "awaiting_payment_confirmation";
                scheduleSave();
                await sendChoiceCard(to, "OTP session expired. Please use Payment Link instead.", [
                    { label: "Payment Link", id: "PAY_LINK" },
                    { label: "Cancel Order", id: "CANCEL_CHECKOUT" },
                    { label: "Menu", id: "SHOW_CATALOG" },
                ], storeName);
            ...
            try {
                ...
                if (!ok) {
                    const msg = typeof json?.message === "string" ? String(json.message) : "OTP submission failed.";
                    await sendChoiceCard(to, `❌ ${msg}\n\nUse Payment Link instead.`, [
                        { label: "Payment Link", id: "PAY_LINK" },
                        { label: "Cancel Order", id: "CANCEL_CHECKOUT" },
                        { label: "Menu", id: "SHOW_CATALOG" },
                    ], storeName);
                ...
                await sendChoiceCard(to, "❌ OTP submission failed. Please use Payment Link instead.", [
                    { label: "Payment Link", id: "PAY_LINK" },
                    { label: "Cancel Order", id: "CANCEL_CHECKOUT" },
                    { label: "Menu", id: "SHOW_CATALOG" },
                ], storeName);
        ...
        const aiFallbackActive = typeof session.aiFallbackAt === "number" && Number.isFinite(session.aiFallbackAt) && nowMs() - session.aiFallbackAt < 30 * 60_000;
        ...
        if (upperText.startsWith("VAR_PAGE ")) {
            ...
            session.pendingConfirm = undefined;
            if (product)
                await sendVariationPicker(to, product, page);
            else
        ...
        if (upperText.startsWith("SELECT_VARIATION ")) {
            ...
            if (product && variation) {
                if (await maybeTriggerHandoff(to, session, vendorCfg, storeName, "select_variation", {
        ...
        if (upperText.startsWith("BUY_VAR ")) {
            ...
            const variation = product?.variations?.find((v) => v.id === vid) ?? null;
            if (product && variation) {
                if (await maybeTriggerHandoff(to, session, vendorCfg, storeName, "select_product", {
                    title: `${product.name} — ${variation.name}`,
                    product_id: product.id,
                    variation_id: variation.id,
                }))
        ...
        if (upperText.startsWith("VIEW_PRODUCT ") || (upperText.includes("VIEW") && /\d/.test(upperText))) {
            ...
            session.pendingConfirm = undefined;
            if (product) {
                if (await maybeTriggerHandoff(to, session, vendorCfg, storeName, "view_product", {
                    title: product.name,
                    product_id: product.id,
                }))
                    return;
        ...
        if (upperText.startsWith("STATUS_PRODUCT ")) {
            ...
            session.pendingConfirm = undefined;
            if (product)
                await sendStatusForwardProduct(to, product);
            else
                await sendWithMenu(to, session, "That product was not found. Tap Menu to browse available items.", storeName);
            return;
        }
        ...
        session.pendingConfirm = undefined;
        session.pendingShopText = undefined;
        const storeFactsParts = [];
        const wa = dmUrlForPeer(shopPhone);
        if (shopPhone)
            storeFactsParts.push(`Shop phone: ${shopPhone}${wa ? ` (${wa})` : ""}`);
        const d = vendorCfg?.deliveryOptions ?? null;
        if (d) {
            const notes = typeof d.notes === "string" ? d.notes.trim() : "";
            if (notes)
                storeFactsParts.push(`Delivery notes: ${notes.slice(0, 500)}`);
