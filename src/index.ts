...
function getHeader(req: express.Request, name: string) {
...
function vendorRateLimit(req: express.Request, res: express.Response, next: express.NextFunction) {
...
function isQrFresh(runtime: { lastQrAt?: number }, maxAgeMs = 25_000) {
...
function waitForVendorEvent(
  runtime: { emitter: NodeJS.EventEmitter },
  timeoutMs: number
): Promise<{ type: "qr"; qr: string } | { type: "status"; status: string } | { type: "error"; message: string } | null> {
  return new Promise((resolve) => {
    ...
    const onEvent = (evt: unknown) => {
      ...
      const e = evt as { type?: unknown; qr?: unknown; status?: unknown; message?: unknown };
...
app.get("/vendors/:vendorId/events", async (req, res) => {
  ...
  try {
    ...
    if (runtime.lastError) writeSseEvent(res, { type: "error", message: runtime.lastError });
    if (runtime.lastQr && isQrFresh(runtime, 60_000)) writeSseEvent(res, { type: "qr", qr: runtime.lastQr });
  } catch (e: unknown) {
...
app.post("/vendors/:vendorId/messages", async (req, res) => {
  ...
  const buttons = Array.isArray(req.body?.buttons) ? (req.body.buttons as unknown[]) : null;
  ...
  const imageUrlsRaw = Array.isArray(req.body?.imageUrls) ? (req.body.imageUrls as unknown[]) : Array.isArray(req.body?.image_urls) ? (req.body.image_urls as unknown[]) : null;
  ...
  const videoUrlsRaw = Array.isArray(req.body?.videoUrls)
    ? (req.body.videoUrls as unknown[])
    : Array.isArray(req.body?.video_urls)
      ? (req.body.video_urls as unknown[])
      : null;
  ...
  try {
    if (imageUrls.length && (videoUrls.length || videoUrl)) {
      ...
      void sendVendorMedia(vendorId, to, imageUrls, vids, cap)
        ...
        .catch((e) => console.log(`vendor ${vendorId}: media error ${(e as any)?.message ?? String(e)}`));
    ...
    if (imageUrls.length) {
      ...
      void sendVendorImages(vendorId, to, imageUrls, cap).then((r) => console.log(`vendor ${vendorId}: images result ${JSON.stringify(r)}`)).catch((e) => console.log(`vendor ${vendorId}: images error ${(e as any)?.message ?? String(e)}`));
      res.status(202).json({ ok: true, queued: true });
      return;
    }
    ...
    if (imageUrl) {
      ...
      void sendVendorImage(vendorId, to, imageUrl, cap).then((r) => console.log(`vendor ${vendorId}: image result ${JSON.stringify(r)}`)).catch((e) => console.log(`vendor ${vendorId}: image error ${(e as any)?.message ?? String(e)}`));
      res.status(202).json({ ok: true, queued: true });
      return;
    }
    ...
    if (videoUrls.length) {
      ...
      void sendVendorMedia(vendorId, to, [], videoUrls, cap)
        ...
        .catch((e) => console.log(`vendor ${vendorId}: videos error ${(e as any)?.message ?? String(e)}`));
      res.status(202).json({ ok: true, queued: true });
      return;
    }
    ...
    if (videoUrl) {
      ...
      void sendVendorVideo(vendorId, to, videoUrl, cap)
        ...
        .catch((e) => console.log(`vendor ${vendorId}: video error ${(e as any)?.message ?? String(e)}`));
      res.status(202).json({ ok: true, queued: true });
      return;
    }
    ...
    if (buttons && buttons.length) {
      const list = buttons
        ...
        .map((b) => ({
          id: typeof (b as any).id === "string" ? (b as any).id.trim() : "",
...
app.put("/vendors/:vendorId/config", async (req, res) => {
  ...
  const keywords =
    escalationKeywords && escalationKeywords.every((x: unknown) => typeof x === "string") ? (escalationKeywords as string[]) : undefined;
...
app.post("/vendors/:vendorId/drops", async (req, res) => {
  ...
  const text = typeof req.body?.text === "string" ? req.body.text : "";
  const lockBefore =
    req.body?.lockBefore === true || req.body?.lock_before === true || req.body?.lock === true ? true : false;
  ...
  try {
    const resp = await postDrop(vendorId, {
      channel,
      targets: targets.filter((x: unknown) => typeof x === "string") as string[],
      text,
      lockBefore,
      unlockAfterSeconds,
      mediaUrl,
...
async function listSessionVendorIds(): Promise<string[]> {
  try {
...
async function isVendorSessionRegistered(vendorId: string): Promise<boolean> {
...
async function bootstrapRuntimes() {
  ...
  for (const vendorId of vendorIds) {
    try {
      ...
      if (!existing) await startVendorRuntime(vendorId);
    } catch (e: unknown) {
...
app.listen(port, host, () => {
  ...
  if (publicUrl) process.stdout.write(`bot-server public url: ${publicUrl}\n`);
  else process.stdout.write(`bot-server public url: (set PUBLIC_BASE_URL to show your domain)\n`);
